...
export default function AppFooter() {
  ...
  const isRecord = pathname?.startsWith("/patterns");
  ...
  return (
    <footer className="sticky bottom-0 z-10 border-t border-slate-200 bg-white">
      <nav className="mx-auto grid max-w-[760px] grid-cols-3 gap-2 px-4 py-2" aria-label="フッターナビ">
        <Link className={`${linkBase} ${isDashboard ? activeClass : inactiveClass}`} href="/">
          <span className="material-symbols-outlined" aria-hidden="true">
            home
          </span>
          <span>ダッシュボード</span>
        </Link>
        <Link className={`${linkBase} ${isRecord ? activeClass : inactiveClass}`} href="/patterns">
          <span className="material-symbols-outlined" aria-hidden="true">
            edit_calendar
          </span>
          <span>記録</span>
